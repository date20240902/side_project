...
### PM 백로그 (우선순위, 미구현)
- **P0 리텐션**: 데일리 알림(카카오 알림톡/푸시: "오늘 문제 열림", "채점결과 N/5") + 연속참여(스트릭). 데일리 게임 생존 직결
- **P0 당첨 루프**: 상품/당첨 탭(치킨풀·규칙·지난 당첨자 공개) + 당첨 통지/수령 흐름. winners 테이블 필요. (탭 추가는 다음 작업 후보 1순위)
- **P1 바이럴**: 결과 자랑 이미지 카드 + **공유 시 예측 페이지 링크 동봉**(canvas 이미지+url). 카카오 SDK 공유 템플릿
- **P2**: 온보딩/룰 1장, 내 기록 보강(정답률·최고기록·스트릭·받은 치킨), 미채점 날 "0/5" 대신 "채점중" 표기, 관중난입 변별력 재검토
...
### 메모 / 남은 검증
- **데모 데이터**: `questions`에 오늘(2026-05-23) published 5문제 + `lock_at = 시드시각+3h`. 정리: `delete from public.questions where match_date = date '2026-05-23';`
