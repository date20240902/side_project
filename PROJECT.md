...
- `preview.html` — 사전예약 랜딩(레트로/픽셀). `/preview`로 보존. **사전예약 = 카카오톡 채널 추가 방식**(카카오 로그인 X, 비즈앱 불필요): 버튼 3개 → 채널 추가 링크 `http://pf.kakao.com/_DqSbX` 열고 안내 모달. 진짜 지표 = **카카오 채널 관리자센터 친구 수**(사이트엔 실시간 숫자 표시 안 함). Supabase 미사용(스크립트 제거됨)
