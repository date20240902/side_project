...
**A. 안정적 50:50**: 양팀 모두 득점(BTTS Y/N) · 합계 2.5골 오버/언더 · 후반 추가시간 5분 이상/이하 · 합산 옐로카드 4장 이상/이하 · 합산 코너킥 10개 이상/이하 · 전반전 골 Y/N · 후반 골>전반 골 Y/N · 선제골 팀 그대로 승리 Y/N

**B. 특이한데 확률 적당(양념)**: VAR 온필드 리뷰 Y/N · 골대(포스트·크로스바) 맞는 슛 Y/N · 헤더골 Y/N · GK 선방 3개 이상/이하 · 교체선수 골·도움(조커) Y/N · 0-0 전반 종료 Y/N · 손흥민 유효슈팅 2개 이상/이하

**C. 잭팟성(낮은 확률, 가끔 1문제)**: 퇴장(레드) Y/N(~15%) · 관중 난입 🎰 · 유니폼 벗기 세리머니 경고 🎰 · 승부차기 진입 🎰(토너먼트)

**하루 5문제 권장 조합**: 경기결과(승무패) 1 + 스타선수(손흥민 등) 1 + 안정 50:50 2 + 잭팟성 1.
**옵션 포맷**: 대부분 `{YES,NO}` 또는 `{이상,이하}`/`{오버,언더}`, 경기결과만 `{승,무,패}`.
