...
## 게임 규칙 (확정)
- 매일 **5문제** 예측(현재 표준 세트): 경기결과(승/무/패), 손흥민 공격포인트(Y/N), 골 더 많은 하프(전반/후반/동률), 양팀 모두 득점 BTTS(Y/N), 퇴장·레드카드(Y/N, 잭팟). **매일 바뀜.** (질문 뱅크 참고. 오버/언더는 토토 느낌이라 미사용)
  - ⚠️ **DB 데모 문제는 아직 옛 세트**(첫 골 시점/PK/관중난입)라 라이브 본 서비스엔 옛 문제가 보임. 데모 데이터 재생성 시 위 표준 세트로 교체할 것(데모 predictions도 함께 정리)
...
- `preview.html` — 사전예약 랜딩(레트로/픽셀). `/preview`로 보존. **사전예약 = 카카오 로그인 방식**(비즈 승인 완료, Supabase Auth Kakao). 가입 버튼 → 카카오 로그인 → 복귀 시 **완료 모달(채널추가=오픈 푸시 / 공유 / 다음에 3버튼)**. 로그인 시 `signups` 테이블 자동 적립(트리거) = 사전예약자 명단·이메일 확보. 채널 공개ID `_DqSbX`(오픈 푸시용, 선택). (이전 채널추가-only 방식은 git 히스토리)
  - **[되살리기 후보, 보류]** "이미 모인 사람들" LIVE STATS 섹션(실시간 참여자 수 + 적립 치킨 + 다음 치킨까지 진행바)은 제거함. 이유: 채널 추가는 클라가 실제 추가 수를 못 읽어 카운터가 0부터 시작하는 클릭 프록시 → "0명"이 역효과. 사람 많이 모이면 되살리기: ⓐ 관리자센터 친구 수를 수동 동기화한 숫자, 또는 ⓑ 서버/비즈앱으로 친구 수 API 연동. 과거 구현(prereg_clicks 테이블+`prereg_count`/`prereg_join` RPC, `ls-*` 마크업/CSS, `renderLive`/`refreshCount` JS)은 **git 히스토리로 복원 가능**(DB 객체는 drop함). 현재는 숫자 빼고 "500명당 +1마리" 스케일링 훅만 정적으로 노출(B안)
...
- `channel-profile.html` — 카카오 채널 프로필 이미지(640×640, 원형 크롭). **새 픽셀/레트로 UI로 재디자인**(네이비 히어로 배경+DungGeunMo 워드마크+픽셀 치킨·축구공 엠블럼). 브라우저로 열어 사각형만 캡처해 업로드. 채널 공개 ID `_DqSbX`
...
**채점 안전 규칙(중요)**
- **오버/언더(토토) 배제**: 수치 라인은 도박 느낌 → 쓰지 않음. **이벤트 Y/N**("일어난다/안 일어난다")과 **카테고리 3지선다** 위주. 경기결과 승/무/패는 OK
- **동률·해당없음 케이스 처리**: 옵션에 포함(3지선다)하거나 **무효(채점 제외)** 명시
- **정성적 판정 배제**: 선방 같은 모호 기준 제외. 공식 기록 or 중계 화면으로 확정 가능한 것만
- **선수 지정 문제**: 해당 선수 **선발 미출전 시 무효(채점 제외)** 명시. 더 안전하게는 **라인업 확정(킥오프 ~1시간 전) 후 공개**해 결장 리스크 제거. ('선발' 기준으로 교체출전 변수도 차단)

**A. 안정적 50:50(이벤트/카테고리형)**: 경기결과 {승/무/패} · BTTS(양팀 모두 득점) Y/N · 전반전 골 Y/N · **골이 더 많은 하프 {전반/후반/동률}** · 선제골 팀 그대로 승리 Y/N(**0-0이면 무효**) · 무승부 Y/N

**B. 특이한데 확률 적당(양념, 이벤트형)**: VAR 온필드 리뷰 Y/N · 골대(포스트·크로스바) 맞는 슛 Y/N · 헤더골 Y/N · 교체선수 골·도움(조커) Y/N · 0-0으로 전반 종료 Y/N

**스타선수형**: 손흥민 공격포인트(골 or 도움) Y/N · 손흥민 골 Y/N (※선발 미출전 시 무효)

**C. 잭팟성(낮은 확률, 가끔 1문제)**: 퇴장(레드) Y/N(~15%) · 관중 난입 🎰 · 유니폼 벗기 세리머니 경고 🎰 · 승부차기 진입 🎰(토너먼트)

※ 폐기: 수치 오버/언더 라인(합계골·추가시간·카드·코너·유효슈팅) — 토토 느낌이라 제외. "GK 선방 N개"는 정성적이라 제외.

**하루 5문제 권장 조합**: 경기결과(승무패) 1 + 스타선수(손흥민 등) 1 + 안정 50:50 2 + 잭팟성 1.
**옵션 포맷**: 대부분 `{YES,NO}`(이벤트형), 비교형은 `{전반,후반,동률}`, 경기결과만 `{승,무,패}`. (오버/언더 미사용)
